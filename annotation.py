...
def annotate_compounds(peaklist, lib_adducts, ppm, db_out, db_name, patterns=True, db_in="", rt_tol=None):
    ...
    def match(db_name: str, db_local: str) -> bool:
        print(f'{repr(db_name)}/{repr(db_local)}')
        temp_db_name = repr(db_name).strip("'")
        temp_db_name = temp_db_name.replace(".sql.gz", "")
        temp_db_name = ''.join([i for i in temp_db_name if i is not "_"])
        temp_db_name = ''.join([i for i in temp_db_name if not i.isdigit()])

        temp_db_local = repr(db_local).strip("'")
    ...
    if db_in is None or db_in == "":
        ...
        if conn_cpds is None:
            raise ValueError("Database {0} not available. db_locals: {1}".format(db_name, str(os.listdir(path_dbs))))
